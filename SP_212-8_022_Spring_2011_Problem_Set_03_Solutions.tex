  ...
  \begin{enumerate}[(a)]
    ...
    \item
      \begin{enumerate}[(i)]
        ...
        \item $$\vec{\nabla}\cdot {\left(  \frac{\hat r}{r^2}  \right)} = \frac{1}{r^2} \dr\left( r^2 \left( \frac{1}{r^2} \right) \right) + \frac{1}{r \sin \theta} \dtheta\left( 0 \right) + \frac{1}{r \sin \theta} \frac{\partial}{\partial \phi}\left( 0 \right) = \frac{1}{r^2}.0 = 0, \text{ for }r \neq  0 $$
          {\color{red} What does $\frac{1}{r^2}.0 = 0$ mean?}
          ...
          Further, for an infinitesimally small sphere $(r \rightarrow 0)$,
          $$  \int \vec{\nabla}\cdot {\left(  \frac{\hat r}{r'^2}  \right)}\,dV \approx \frac{4}{3} \pi r^3  \vec{\nabla}\cdot\left(\frac{\hat r}{r^2}\right) $$
          It is clear from the above equation and \autoref{eq:Glaw} that the divergence of $\hat r/r^2$ diverges at the origin. Hence,
          $$\vec{\nabla}\cdot {\left(  \frac{\hat r}{r^2}  \right)} = 4 \pi \delta^3(\vec{r}) $$
          where, $\delta^3(\vec{r})$ is the three dimensional Dirac delta function defined as follows:
    ...
    \item
      \begin{enumerate}[(i)]
        ...
        \item $${\vec{\nabla} \times \left( \frac{ \hat\phi}{R} \right)} = \hat{R} \left( 0 -0 \right) + \hat\phi \left( 0 - 0\right) + \hat z \left( \frac{1}{R} \dr \left( \frac{1}{R} \right) - 0 \right) = \frac{1}{R}.0 = 0\text{ for }R \neq 0 $$
...
\subsection{Solution}
  \begin{enumerate}
    ...
    \item
      ...
      where (for this problem) the surface area element is $d\vec{S}$ instead of the usual $d\vec{A}$ since the vector $\vec{A}$ has already been assigned a different meaning. We can evaluate the surface integral by summing up the surface integral over several infinitesimal cuts (Please see \autoref{fig:balloon}).
      Hence,
      $$\oint  \left( \vec{\nabla} \times \vec{A} \right)\cdot \hat{n}\,dS = \sum_i \int_{\operatorname{int}C_i}\left( \vec{\nabla} \times \vec{A} \right)\cdot \hat{n}\,dS$$
      where $\operatorname{int}C_i$ denotes the region enclosed by $C_i$ on the curved surface (white area). Using Stoke's law we get,
      $$\sum_i \int_{\operatorname{int}C_i}\left( \vec{\nabla} \times \vec{A} \right)\cdot \hat{n}\,dS = \int_{C_i} \vec{A}\cdot \vec{dr}$$
...
\subsection{Solution}
  \begin{enumerate}[(a)]
    ...
    \item
      \begin{enumerate}[(i)]
        \item By Gauss' Law: \par\noindent
          \begin{figure}[h]
            \centering
            \includegraphics[width=0.4\textwidth]{ps03_sol_05}
            \caption{Gaussian Surface (a) Outside (b) Inside}
            \label{fig:cylinder}
          \end{figure}
          Note that the charge distribution is axially symmetric. Hence, there is no dependence on $\phi$. Since the cylinder is infinite, there is no dependence on $z$. Further, we must also have $E_\phi = E_z = 0$. Hence, $E_r$ is the only non-zero component of the electric field and it is non-zero.\footnote{This $r$ is the radial coordinate of cylindrical polar coordinates} The electric field outside the cylinder can be found by applying Gauss's law to the Gaussian surface (cylinder) shown in \autoref{fig:cylinder} (a):
          $$E_r \left( 2\pi r h\right) = 4 \pi \left( \rho \pi R^2 h\right)$$
          Hence, the electric field outside the cylinder is
          $$E_r = \frac{2 \pi \rho R^2}{r}$$
          Similarly, the electric field inside the cylinder is found by applying Gauss's law to the Gaussian surface in Figure (\ref {cylinder} b)
          $$E_r \left( 2\pi r l\right) = 4 \pi \left( \rho \pi r^2 l\right) $$
          Hence, the electric field inside the cylinder is
          $$E_r = {2 \pi \rho r }$$
        \item By integration: \par\noindent
          Outside the cylinder the differential form of Gauss's law takes the following form (in cylindrical coordinates)
...
\subsection{Solution}
  \begin{enumerate}[(a)]
    ...
    \item The expression for the dipole potential in \autoref{eq:potdipsim} can be written in spherical polar coordinates as follows\footnote{Please see handout.},
...
\subsection{Solution}
  ...
  $$\vec{E}(r) =
    ...
    \end{cases}
  $$
  where $\vec{r}$ denotes the position vector of the point at which electric field is evaluated and $\vec{r}_{0}$ denotes the position vector of the center of the spherical shell.
...
\subsection{Solution}
  ...
  In SI units,
  \[ \phi(0) = \frac{1}{4\pi\epsilon_0}\frac{3Q}{2a} = \frac{3\cdot79(\SI{1.6e-19}{\coulomb})}{4\pi(\SI{8.85e-12}{\coulomb\squared\per\newton\per\meter\squared})2(6\cdot19^{-15}\si{\meter})} = \SI{28.4}{\mega\volt}. \] % times 19^{-15}!?
  {\color{red} Is it really $6\cdot19^{-15}\si{\meter}$ and not \SI{6e-15}{\meter}?}
\end{document}
